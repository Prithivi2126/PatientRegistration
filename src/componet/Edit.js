...
import { putDataRequest } from "../redux/action/action";
import "./style.css";
...
const Edit = () => {
  ...
  useEffect(() => {
    const itemToEdit = state.array.find((item) => item.id === id);
  ...
  const initialValues = {
    ...
    emergencyno: formData ? formData.emergencyno : "",
    primaryPhysician: formData ? formData.primaryPhysician : "",
  };
